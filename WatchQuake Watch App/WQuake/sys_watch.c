...
void Sys_ExpandCommandLine(const char* commandLine)
{
    ...
    memcpy(sys_commandline, "WQuake ", 7);
    memcpy(sys_commandline + 7, commandLine, length);
    
    qboolean atwhitespace = qFalse;
...
{
    com_skipbasedir = qFalse;
    //com_skipbasedir = qTrue;
    ...
    memset(com_savedir,0,MAX_OSPATH);
    memcpy(com_savedir,saveDir,savedirlength);
    ...
    parms.basedir = sys_resourcesdir;//sys_documentsdir;
...
int Sys_ShowFPS()
{
    return scr_showfps.value > 0.5f ? 1 : 0;
}
