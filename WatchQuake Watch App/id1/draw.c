...
void Draw_SubPic (int x, int y, int offX, int offY, int w, int h, qpic_t *pic)
{
    byte            *dest, *source;
    unsigned short    *pusdest;
    int                v, u;
...
void Draw_SubPicDest (int x, int y, int offX, int offY, int w, int h, qpic_t *pic, qpic_t* destPic)
{
    byte            *dest, *source;
    unsigned short    *pusdest;
    int                v, u;
    ...
    int rowbytes = destPic->width;
    if (r_pixbytes == 1)
    {
        dest = destPic->data + y * rowbytes + x;
...
void Draw_TransSubPic (int x, int y, int offX, int offY, int w, int h, qpic_t *pic)
{
    byte    *dest, *source, tbyte;
    unsigned short    *pusdest;
    int                v, u;
...
// hack the version number directly into the pic
    ...
    dest = conback->data + 197 + 8 + 320*186;
	sprintf (ver, "%4.2f", VERSION);
#endif

    Draw_SubPicDest(198 + 8,159,278,159,29,24,conback,conback);
    Draw_SubPicDest(196 + 8,187,276,187,33,8,conback,conback);
...
// draw the pic
	if (r_pixbytes == 1)
	{
        ...
        float aspect = ((float)(vid.conheight)) / ((float)(vid.conwidth));
		for (y=0 ; y<lines ; y++, dest += vid.conrowbytes)
		{
			...
			else
			{
                ...
                f = 60*0x10000;
                fstep = 320*0x10000/(vid.conheight * aspect);
        ...
        //Draw_TransSubPic(198,159,278,159,29,24,conback);
	}
	else
	{
		...
		// FIXME: does the endian switching go away in production?
			...
			f = 80*0x10000;
			//fstep = 320*0x10000/vid.conwidth;
            fstep = 320*0x10000/vid.conheight;
			for (x=0 ; x<vid.conwidth ; x+=4)
			{
