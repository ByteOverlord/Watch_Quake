...
//#define MENU_Y_OFFSET 130
#define MENU_Y_OFFSET (vid.height / 3)
...
void M_DrawTransSubPic (int x, int y, int offX, int offY, int w, int h, qpic_t *pic)
{
    y += MENU_Y_OFFSET;
    Draw_TransSubPic (x + ((vid.width - 320)>>1), y,offX,offY,w,h,pic);
}

void M_DrawSubPic (int x, int y, int offX, int offY, int w, int h, qpic_t* pic)
{
    y += MENU_Y_OFFSET;
    Draw_SubPic (x + ((vid.width - 320)>>1),y,offX,offY,w,h,pic);
}
...
void M_Main_Draw (void)
{
    ...
    M_DrawTransSubPic(72,32,0,0,p->width,20,p);// SinglePlayer
    
    M_DrawTransSubPic(72,32 + 20,0,20,23,18,p);// M
    M_DrawTransSubPic(72 + 23,32 + 22,122,22,18,14,p);// A
    M_DrawTransSubPic(72 + 23 + 18,32 + 23,86,23,19,13,p);// P
    M_DrawTransSubPic(72 + 23 + 18 + 19,32 + 23,106,43,17,13,p);// S
    
    M_DrawTransSubPic(72,32 + 40,0,40,p->width,41,p);// Options, Help

    M_DrawTransSubPic(72,32 + 81,0,41,11,16,p);// C (O)
    M_DrawTransSubPic(72 + 11,32 + 81,192,2,7,5,p);// C (E)
    M_DrawTransSubPic(72 + 11,32 + 81 + 11,192,10,7,5,p);// C (E)
    M_DrawTransSubPic(72 + 18,32 + 81 + 2,57,63,8,13,p);// H (P)
    M_DrawTransSubPic(72 + 18 + 8,32 + 81 + 2,59,43,8,13,p);// H (I)
    M_DrawTransSubPic(72 + 18 + 8 + 8,32 + 81 + 2,154,23,16,13,p);// E
    M_DrawTransSubPic(72 + 18 + 8 + 8 + 16,32 + 81 + 1,122,22,18,14,p);// A
    M_DrawTransSubPic(72 + 18 + 8 + 8 + 16 + 18,32 + 81 + 2,52,83,18,16,p);// T
    M_DrawTransSubPic(72 + 18 + 8 + 8 + 16 + 18 + 18,32 + 81 + 2,106,43,17,13,p);// S
...
int m_cheats_cursor = 0;
int m_cheats_showfps = 0;

#define CHEATS_ITEMS 5
int m_cheats_types[] =
{
    2,
    2,
...
char* m_cheats_names[][2] =
{
    {"god\n",          "              God Mode"},
    {"notarget\n",     "             No Target"},
    {"fly\n",          "                   Fly"},
    {"r_drawviewmodel","            View Model"},
    {"impulse 9\n",    "      Give All Weapons"},
};
...
void M_SetCheatState(int valueIndex, int cheatflag)
{
    char buffer[256];
    const char* cmdTxt = NULL;
    if (m_cheats_types[valueIndex] > 1)
    {
        if (!sv_player)
        {
            return;
        ...
        int value;
        int cheatValue = m_cheats_values[valueIndex];
        if (m_cheats_types[valueIndex] == 3)
        {
            value = (int)sv_player->v.movetype;
            cheatValue = cheatValue ? MOVETYPE_FLY : MOVETYPE_WALK;
        }
        else
        {
            value = ((int)sv_player->v.flags) & cheatflag ? 1 : 0;
        }
        if (cheatValue != value)
        {
            cmdTxt = m_cheats_names[valueIndex][0];
        }
    }
    else
    {
        ...
        if (m_cheats_types[valueIndex] == 1)
        {
            snprintf(buffer,256,"%s %i\n",m_cheats_names[valueIndex][0],m_cheats_values[valueIndex]);
            cmdTxt = buffer;
        }
    }
    if (cmdTxt)
    {
        Cmd_ExecuteString(cmdTxt,src_command);
    }
}

void M_OnPlayerSpawn(void)
{
...
{
    int cheatFlags[] = {FL_GODMODE,FL_NOTARGET,FL_FLY};
    switch (index)
    {
        case 0:
        case 1:
        case 2:
            M_SetCheatState(index,cheatFlags[index]);
        break;
        default:
            M_SetCheatState(index,0);
            break;
    };
}
...
void M_Cheats_Draw (void)
{
    for (int i=0; i<CHEATS_ITEMS; i++)
    {
        M_Print(16, 32 + i * 8,m_cheats_names[i][1]);
        if (m_cheats_types[i])
        {
            M_DrawCheckbox(220,32 + i * 8,m_cheats_values[i]);
        }
    }
    // cursor
    M_DrawCharacter (200, 32 + m_cheats_cursor*8, 12+((int)(realtime*4)&1));
}
...
int M_IsCheatActive(int i)
{
    if (i < CHEATS_ITEMS)
    {
        if (m_cheats_types[i] == 1)
        {
            return m_cheats_values[i];
        }
    }
    return 0;
}
...
void M_Cheats_Key (int key)
{
    ...
    case K_DOWNARROW:
        S_LocalSound ("misc/menu1.wav");
        if (++m_cheats_cursor >= CHEATS_ITEMS)
            m_cheats_cursor = 0;
        break;
    case K_UPARROW:
        S_LocalSound ("misc/menu1.wav");
        if (--m_cheats_cursor < 0)
            m_cheats_cursor = CHEATS_ITEMS - 1;
        break;
    case K_ENTER:
        S_LocalSound ("misc/menu2.wav");
        if (m_cheats_types[m_cheats_cursor] != 0)
        {
            m_cheats_values[m_cheats_cursor] = m_cheats_values[m_cheats_cursor] ? 0 : 1;
        }
        M_OnCheatStateChange(m_cheats_cursor);
    }
}
...
{
    int prevChanged = changedSettings;
    changedSettings = 1;
	S_LocalSound ("misc/menu3.wav");
    ...
    case 12:
        ...
        break;
    case 13:
        Cvar_SetValue("r_lerpmove",!r_lerpmove.value);
        break;
    case 14:
        Cvar_SetValue("scr_showfps",!scr_showfps.value);
        changedSettings = prevChanged;
        break;
/*#ifdef _WIN32
...
void M_Options_Draw (void)
{
    ...
    M_Print (16, 128, "      Smooth Animation");
    M_DrawCheckbox (220, 128, r_lerpmodels.value);

    M_Print (16, 136, "       Smooth Movement");
    M_DrawCheckbox (220, 136, r_lerpmove.value);

    M_Print (16, 144, "              Show fps");
    M_DrawCheckbox (220, 144, scr_showfps.value);
...
void M_Options_Key (int k)
{
	...
	case K_ENTER:
		...
		case 2:
            ...
            COM_SetConfigOverride(1);
			Cbuf_AddText ("exec config.cfg\n");
            Cbuf_Execute();
            COM_SetConfigOverride(0);
            changedSettings = 1;
			break;
		/*case 12:
			M_Menu_Video_f ();
...
OnMenuNavigate M_EnterMapsFunc = NULL;
OnMenuNavigate M_ExitMapsFunc = NULL;
OnMenuNavigate M_SelectMapsFunc = NULL;
...
{
    return key_dest == key_menu && m_state == m_multiplayer;
}

int m_maps_selectedDifficulty = 0;
...
{
    return levels[m_maps_selectedMap+1].name;
}
...
{
    //for (int i=0; i<MAPS_ITEMS; i++)
    {
        //M_Print(16, 32 + i * 8,levels[i+1].description);
        M_Print(64, 32 + 4 * 8 - MENU_Y_OFFSET,levels[m_maps_selectedMap+1].name);
        M_Print(64 + 8 * 8, 32 + 4 * 8 - MENU_Y_OFFSET,levels[m_maps_selectedMap+1].description);
    }
    M_Print(64, 32 + 5 * 8 - MENU_Y_OFFSET,"Skill");
    M_Print(64 + 8 * 8, 32 + 5 * 8 - MENU_Y_OFFSET,difficultyLevels[m_maps_selectedDifficulty]);
    M_Print(64, 32 + 6 * 8 - MENU_Y_OFFSET,"Start");
    // cursor
...
void M_Maps_Key (int key)
{
    switch (key)
    {
        ...
        case K_DOWNARROW:
            S_LocalSound ("misc/menu1.wav");
            if (++m_maps_cursor >= MAPS_ITEMS)
                m_maps_cursor = 0;
            break;
        case K_UPARROW:
            S_LocalSound ("misc/menu1.wav");
            if (--m_maps_cursor < 0)
                m_maps_cursor = MAPS_ITEMS - 1;
            break;
        case K_LEFTARROW:
            S_LocalSound ("misc/menu1.wav");
            if (m_maps_cursor == 0)
            ...
            else if (m_maps_cursor == 1)
            {
                if (--m_maps_selectedDifficulty < 0)
                {
                    m_maps_selectedDifficulty = 3;
                }
            }
            break;
        case K_RIGHTARROW:
            ...
            {
                S_LocalSound ("misc/menu1.wav");
                if (++m_maps_selectedDifficulty > 3)
                {
        ...
        case K_ENTER:
            if (m_maps_cursor == 2)
            {
                ...
                Cbuf_AddText ("maxplayers 1\n");
                char mapStr[128];
                snprintf(mapStr,128,"skill %i\nmap %s\n",m_maps_selectedDifficulty,levels[m_maps_selectedMap+1].name);
                Cbuf_AddText (mapStr);
            }
            break;
        default:
...
void M_Init (void)
{
    ...
    OnPlayerSpawnFunc = M_OnPlayerSpawn;
