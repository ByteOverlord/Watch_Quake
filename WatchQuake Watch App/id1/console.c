...
void Con_DrawNotify (void)
{
	...
	for (i= con_current-NUM_CON_TIMES+1 ; i<=con_current ; i++)
	{
		...
		for (x = 0 ; x < con_linewidth ; x++)
            Draw_CharInSequence ( ((x+1)<<3) + NOTIFY_X_OFFSET, v + NOTIFY_Y_OFFSET, text[x]);
	...
	if (key_dest == key_message)
	{
		...
		while(chat_buffer[x])
		{
			Draw_CharInSequence ( ((x+5)<<3) + NOTIFY_X_OFFSET, v + NOTIFY_Y_OFFSET, chat_buffer[x]);
			x++;
		}
		Draw_CharInSequence ( ((x+5)<<3) + NOTIFY_X_OFFSET, v + NOTIFY_Y_OFFSET, 10+((int)(realtime*con_cursorspeed)&1));
		v += 8;
	}
