...
qboolean	r_recursiveaffinetriangles = qTrue;
int			r_pixbytes = 1;
...
cvar_t    r_lerpmodels = {"r_lerpmodels", "0",CVAR_ARCHIVE};
...
// bottom side clip
	...
	if (scr_fov.value <= 90.0)
		r_fov_greater_than_90 = qFalse;
	else
		r_fov_greater_than_90 = qTrue;
...
// FIXME: remove and do real lighting
	...
	for (i=0 ; i<cl_numvisedicts ; i++)
	{
		...
		case mod_alias:
            R_SetupEntityTransform(currententity,&lerpData);
			//VectorCopy (currententity->origin, r_entorigin);
            ...
            {
                b = R_AliasCheckBBoxLerp (&lerpData);
            }
		...
		// trivial accept status
			if (b)
			{
                j = R_LightPoint(lerpData.origin);// (currententity->origin);
				...
				for (lnum=0 ; lnum<MAX_DLIGHTS ; lnum++)
				{
					if (cl_dlights[lnum].die >= cl.time)
					{
						/*VectorSubtract (currententity->origin,
										...
										dist);*/
                        VectorSubtract (lerpData.origin,
                                        cl_dlights[lnum].origin,
                                        dist);
			...
			// clamp lighting so it doesn't overbright as much
                ...
                R_AliasDrawModel (&lighting,&lerpData);
			}
...
#endif
    lerpdata_t lerpData;
    VectorCopy(modelorg,lerpData.origin);
    VectorCopy(currententity->angles,lerpData.angles);
    R_AliasFrameLerp();
	R_AliasDrawModel (&r_viewlighting,&lerpData);
}
